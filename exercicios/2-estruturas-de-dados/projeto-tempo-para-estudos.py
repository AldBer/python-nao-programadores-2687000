...
total_dias = input('Quantos dias na semana você dedica ao estudo?')
total_horas = input('Quantas horas por + / - você estuda por dia?')
curso = input('Qual curso você está estudando?')
total_dias = int(total_dias)
total_horas = int(total_horas)

print (f'Olá {nome}, você costua estudar {total_dias} dias por semana,durante aproximadamente {total_horas} horas diarias. Isso significa que você estuda em média {total_dias * total_horas} horas por semana do curso de {curso}.')